...
Most times it isn't a big deal, you just go with Typescript, install `reflect-metadata` and drop some configs under `tsconfig.json`.

But what if we could just ignore the use of decorators, classes and reflect-metadata? 

That's where Aquinas comes in.
